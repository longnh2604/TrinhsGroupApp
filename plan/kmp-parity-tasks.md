...
| ✅ K-16 | FCM: messaging service, register on login / unregister on logout, persisted `NotificationStore` equivalent, bell + unread badge, tap → order detail (incl. cold launch) | done 2026-08-21. `PushMessagingService` + `PushTokens` (androidApp/firebase), shared `NotificationStore` replaces `NotificationsRepository`, `NotificationsScreen`, `HistoryViewModel.openOrder(orderId)`. That gap closed 2026-08-22 on the server: `trinh-push-notify` 1.4.0 sends Android a data-only message and iOS an `apns.payload.aps.alert`, so `onMessageReceived` runs in every state and every entry carries its order id. iOS unchanged — `getDeliveredNotifications()` already handed back the whole `data` block as `userInfo`, which is why only Android ever had the gap |
